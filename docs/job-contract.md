...
`POST /jobs/<jobId>/runs/<runId>/promotion` creates a proposal after rechecking the supplied `reviewedManifestHash` and all existing promotion gates. `GET /regression-proposals/<runId>` returns the proposed file contents, exact patch, patch SHA-256, configured target repository, and review status. `POST /regression-proposals/<runId>/apply` requires the displayed `reviewedPatchSha256`; it revalidates proposal integrity and applies new regression files to the configured repository without staging, committing, pushing, or opening a pull request.
