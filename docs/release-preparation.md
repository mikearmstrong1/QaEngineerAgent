...
- Publication and local Compose deployment: complete
...
The [candidate record](deployment-candidate.json) contains the registry-qualified immutable reference and deployment evidence. On 2026-09-19 the API and worker were replaced with that digest after a PostgreSQL backup and clean drain. Migrations 1–3 applied, authentication checks passed, retained PostgreSQL jobs and MinIO health were verified, and all five browser/API smoke tests passed.
